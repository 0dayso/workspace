   ...
   <script id="tpl_list_table" type="text/html">
	 <table  width="100%" border="0" cellpadding="0" cellspacing="0" class="list_table">
	   ...
	   {{# for(var i = 0, len = d.list.length; i < len; i++){ }}
		...
		<td class="td_center">{{ $.nullToEmpty(d.list[i].ex.SQ_USERID.xm)}}/{{ $.dateF(d.list[i].SQ_DATETIME,'MM-dd HH:mm')}}</td>	
          ...
          <div class="box_border">
            <div class="box_center pt10 pb10">
               <form action="${ctx}/vedsb/jpzwgl/jptjsq/shZwPagelist" id="searchForm" name="searchForm" method="post">
	           	<input type="hidden"  name="VIEW" value="F914676E7ECFD6E6C59F1DF06AADD404" id="view"/>
	            <input type="hidden"  name="pageNum" value="${param.pageNum }" id="pageNum"/>
				<input type="hidden"  name="pageSize" value="10" id="pageSize"/>
				<input type="hidden" name="lb" value="zwcgqx"/>
				<input type="hidden" value="3" name="cjrclzt"/>
	              <table class="table01" border="0" cellpadding="0" cellspacing="0">
		             <tr>
						<td class="xsys" style="text-align: right;">日期条件</td>
						<td>
							<select name="rqlx" style="width: 100px;height: 24px;">
						...
						<td>
							<input type="text" name="ksrq" value="${empty param.ksrq ? vfn:dateShort() : param.ksrq}" onClick="WdatePicker()" style="width: 96px;height: 20px;" size ="10" />
						</td>
						<td class="xsys" style="text-align: right;">追位日止</td>
						<td>
							<input type="text" name="jsrq" value="${empty param.jsrq ? vfn:dateShort() : param.jsrq}" onClick="WdatePicker()" style="width: 96px;height: 20px;" size ="10">
						</td>
						<td class="xsys" style="text-align: right;">申请人</td>
						<td>
							<input type="text" id="usermc" name="usermc" value="${param.usermc}" style="width: 96px;height: 20px;" size=10/>
						</td>
						<td class="xsys" style="text-align: right;">原PNR</td>
						<td>
							<input type="text"  name="ypnrno" value="${param.ypnrno }" onblur="value=$.trim(this.value).toUpperCase();" style="width: 96px;height: 20px;" size=10/>
						</td>
					</tr>
					<tr>
						...
						<td>
							<input type="text" name="lxr" value="${param.lxr }" onkeyup="this.value=this.value.replace('%','').toUpperCase()" style="width: 96px;height: 20px;" size=10>
						</td>		
						<td class="xsys" style="text-align: right;">联系电话</td>
						<td>
							<input type="text" name="lxsj" value="${param.lxsj }" onkeyup="this.value=this.value.replace('%','').toUpperCase()" style="width: 96px;height: 20px;" size=10 >
						</td>
					...
					<tr>
						...
						<td>
							<input type="text" size="10" name="hbh" value="${param.hbh }" style="width: 96px;height: 20px;"  onkeyup="this.value=this.value.replace('%','').toUpperCase()" >
						</td>		
						<td class="xsys" style="text-align: right;">现舱位</td>
						<td>
							<input type="text" name="xcw" value="${param.xcw }" style="width: 96px;height: 20px;"  onkeyup="this.value=this.value.replace('%','').toUpperCase()" size=10>
						</td>
