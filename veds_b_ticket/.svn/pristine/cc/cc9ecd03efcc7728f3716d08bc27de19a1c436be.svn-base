...
<table class="t_tab2" cellpadding="0" cellspacing="0" align="center" style="width: 100%;margin-bottom: 1px; background:#f1f1f1">
	...
	<tr>
	  <c:if test="${not empty srcs and srcs ne '0'}">
		  <td rowspan="3" style="background:#efefef;height:90px; width:100px; border:1px;text-align:center;">
		 	 ...
		 	 <font color="#3e75d0"><b>${jpkhdd.EX.WDID.wdmc }</b></font>
		  </td>
	  </c:if>
	  ...
	  <td class="wb_td03" style="border:none;">
		...
		<span class="fl">
		  	 <lable class="headtitle">政策代码</lable>
		 	 <span class="wb_td02">${jpkhdd.WD_ZCDM}</span>
	 	 </span>
	 	 <span class="fl">
		  	 <lable class="headtitle">&nbsp;&nbsp;外部单号</lable>
		  	 <span class="wb_td02">
				<input type="text" name="wbdh" value="${jpkhdd.WBDH }"  datatype="*1-60" size="10"/>
	...
	<tr>
	  <td style="border:none">
		...
		<span class="wb_td02"> 
			<!--${jpkhdd.XS_PNR_NO}  -->
			<c:if test="${empty jpkhdd.CG_PNR_NO or jpkhdd.XS_PNR_NO eq jpkhdd.CG_PNR_NO}">${jpkhdd.XS_PNR_NO}</c:if>
			<c:if test="${not empty jpkhdd.CG_PNR_NO and jpkhdd.XS_PNR_NO ne jpkhdd.CG_PNR_NO}"><span class="red">${jpkhdd.XS_PNR_NO}</span><font color="green">->${jpkhdd.CG_PNR_NO}</font></c:if>
		</span>
		
		<span class="headtitle">大编码 </span>
		<span class="wb_td02"> 
			<!-- ${jpkhdd.XS_HKGS_PNR}  CG_HKGS_PNR -->
			<c:if test="${empty jpkhdd.CG_HKGS_PNR or jpkhdd.XS_HKGS_PNR eq jpkhdd.CG_HKGS_PNR}">${jpkhdd.XS_HKGS_PNR}</c:if>
			<c:if test="${not empty jpkhdd.CG_HKGS_PNR and jpkhdd.XS_HKGS_PNR ne jpkhdd.CG_HKGS_PNR}"><span class="red">${jpkhdd.XS_HKGS_PNR}</span><font color="green">->${jpkhdd.CG_HKGS_PNR}</font></c:if>
		</span>
	 	...
	 	<span class="headtitle">PNR </span>	
	 	<span class="wb_td02"> ${jpkhdd.XS_PNR_ZT}</span>
		
		<span class="headtitle">订单状态 </span>	
	 	<span class="wb_td02"> 
		...
		<c:if test="${not empty jpkhdd.CGLY and jpkhdd.CGLY ne 'BOP'}">
			<span class="headtitle">采购来源</span>	
		 	<span class="wb_td02">${jpkhdd.CGLY} </span>
			...
			<span class="wb_td02">
				<c:choose>
					...
					<c:when test="${jpkhdd.CGZT eq '5'}">已取消</c:when>
				</c:choose>
			</span>
	 	</c:if>
	 	<c:if test="${jpkhdd.sfzdd eq '1'}">
