		...
		$(function(){
			...
			validator = $("#b_pc_th_form").Validform({
				...
				callback : function(transport) {
					if (transport.CODE == "0") {
						alert("报废成功");
						window.location.href='${ctx}/vedsb/pzzx/pzbf/viewlist?title=2'
					} else {
						alert(transport.MSG);
					}
				},
		...
		function click_check(startno,endno,sysl,pztype,officeid,xh,isChecked,obj){
			...
			if (isChecked) {
				if ($("input[name='xcd_cb']:checked").length == 1) {
					...
					$.ajax({
						url : "viewbf_xcd_tr?startno=" + startno + "&endno=" + endno + "&sysl=" + sysl + "&pztype=" + pztype+"&officeid="+officeid + "&xh="+ xh,
						cache : false,
...
<!-- 模板 -->
   <script id="tpl_list_table" type="text/html">
	 <table  class="list_table" style="width:1024px;" border="0" cellpadding="0" cellspacing="0" >
       <tr>
		<th width="5">
			<input type="checkbox" id="checkAll" name="checkallbox" onclick="checkAll(this);">
		</th>
        <th width="5">序号</th>
		<th width="8">票证类型</th>
        <th width="17">起始号码</th>
...
<body>
	...
	<div class="container_clear">
       		...
       		<div class="box">
          		<div class="box_border">
            		<div class="box_center pt10 pb10">
            			<form action="${ctx}/vedsb/pzzx/pzbf/bf_xcd_add" id="searchForm" name="searchForm" method="post">
              				...
              				<tr>
              					<td class="text_right">票证类型：</td>
              					  ...
              					  <td class="text_right">
              						...
              						<td class="text_left">
              							<input type="text" id="startno" name="startno" class="input-text lh25"  size="17" width="100px" value="" size="10">
              						</td>
              						<td class="text_right">
              							终止号码：
              						</td>
              						<td class="text_left">
              							<input type="text" id="endno" name="endno" class="input-text lh25"  size="17" width="100px" value="" size="10">
              						</td>
     ...
     <!--  保存区 -->
  <div id="save_div">
  <form action="${ctx}/vedsb/pzzx/pzbf/save_list_jpPzbf" method="post" id="b_pc_th_form" name="b_pc_th_form">
  	<div style="background-color:#fff;">
					<table id="tab" style="width:800px;border-collapse:   separate;   border-spacing:   10px;">
					<tr>
						<td colspan="6" style="color:#FF0000">
							&nbsp;选定页面上边 
							<input type="checkbox" name="123" style="vertical-align:middle;" checked disabled="disabled">复选框，选取发放单号段。可以选择多个号段一起报废。
						</td>
					</tr>
					<tr id="tr_1">
						<td width="66px;">起始单号：</td>
						<td width="170px;">
							<input type="text" id="startno_1" class="input-text lh25"  size="17" width="100px" datatype="integer,maxvalue,minvalue" name="startno" onblur="calcnum(this)"> 
							<font color="red">*</font>
						</td>
						<td width="66px;">终止单号：</td>
						<td width="170px;">
							<input type="text" id="endno_1" class="input-text lh25"  size="17" width="100px" datatype="integer,maxvalue,minvalue" name="endno"onblur="calcnum(this)"> 
							<font color="red">*</font>
						</td>
						<td>数 &nbsp;量：</td>
						<td width="170px;"><input type="text" id="sysl_1" class="input-text lh25" name="sysl"value="0" size="2" readOnly="true"> 
