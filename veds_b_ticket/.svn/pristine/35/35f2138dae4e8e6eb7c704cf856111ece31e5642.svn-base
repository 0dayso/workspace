...
<table class="no_border_table" cellpadding="0" cellspacing="0" border="0">
	...
	<tr>
		...
		<td colspan="2">
			...
			<font color="red">*</font>
			<input type="text" id="commission_${tkno}" name="commission" class="no_border" 
				value="${tfd.cj_dlf}" style="width:160px;text-align: right;" readonly />
		</td>
		<td>
			<font color="red">*</font>
		</td>
		...
		<tr>
			...
			<c:forEach begin="0" end="2">
				...
				<td>
					...
					<c:if test="${empty taxType and v eq 0}">
						<c:set var="taxType" value="CN" />
					</c:if>
					<c:if test="${empty taxType and v eq 1}">
						<c:set var="taxType" value="YQ" />
					</c:if>
					<font color="red">*</font>
