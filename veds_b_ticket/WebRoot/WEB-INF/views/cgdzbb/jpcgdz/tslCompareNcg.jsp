			...
			<display:column title="序号" style="width: 20px">${ticketInfo_rowNum}</display:column>
				<display:merge title="系统数据">
					<display:column title="类型" style="text-align:left;" expfield="{pzlx,1,正常票,2,退票,3,废票}" >
					 ...
					 <display:column title="账单价<br>实退金额" property="xtzdj" format="{0,number,#0.00}" style="text-align:right;${ticketInfo.color1 eq '1' ? 'background-color:red;' : ''}" />
				     <display:column title="税费" style="text-align:right;${ticketInfo.color2 eq '1' ? 'background-color:red;' : ''}" expfield="xttax" property="xttax"></display:column>
					 <display:column style="text-align:right;${ticketInfo.color3 eq '1' ? 'background-color:red;' : ''}" title="代理费率" expfield="xtdlfl" property="xtdlfl"></display:column>
		...
		<display:column style="width:3">
			<div>&nbsp;&nbsp;</div>
				...
				<display:merge title="TSL账单数据">
					...
					<display:column title="账单价/实退金额<br>COLLECTION" property="tslzdj" style="text-align:right;${ticketInfo.color1 eq '1' ? 'background-color:red;' : ''}"/>
					<display:column title="税费" property="tsltax" style="text-align:right;${ticketInfo.color2 eq '1' ? 'background-color:red;' : ''}"/>
					<display:column title="C值" property="tsldlfl" style="text-align:right;${ticketInfo.color3 eq '1' ? 'background-color:red;' : ''}"/>
