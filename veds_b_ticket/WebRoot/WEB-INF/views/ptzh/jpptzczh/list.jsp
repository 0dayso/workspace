...
<style type="text/css">
    ...
    #box li {margin:2px;padding:5px;float:left;width:110px;height:20px;border:1px solid #1D79C7; cursor: pointer;}
    #box .once {background-color: #1D79C7}
</style>
<script type="text/javascript">
	...
	function toSave(){
	    ...
	    var url="${ctx}/vedsb/ptzh/jpptzczh/saveZh";
	...
	function getpaySignOnUrl(_url,_name,_pwd,_emAccount){
		...
		var url = "${ctx}/vedsb/ptzh/jpptzczh/getpaySignOnUrl?url="+_url+"&name="+_name+"&pwd="+_pwd+"&emAccount="+_emAccount;
		var ii = layer.load("系统正在处理您的操作,请稍候!");
		$.ajax({
			url:url,
			type:"post",
	...
	function liclick(ptzcbs) {
		window.location.href = "${ctx}/vedsb/ptzh/jpptzczh/viewlist?ptzcbs=" + ptzcbs;
	}
</script>
</head>
<body>
	...
	<c:forEach items="${list}" var="one">
			<c:if test="${fn:contains(ptzcbs,one.ptzcbs)}">
			  	<div id="box">
				  	<li onclick="liclick('${one.ptzcbs }')" class="${one.ptzcbs eq param.ptzcbs ? 'once' : '' }">
				  		<c:forEach items="${vfc:getVeclassLb('100021')}" var="pt" varStatus="wdptStatus">
                  	 		 <c:if test = "${one.ptzcbs eq pt.ywmc}">
                  	 			${pt.mc }
                  	 		 </c:if>
                  	 	</c:forEach>
	        			<b style="color:${one.open1 eq '1' ? 'green' : 'red'}">(${one.open1 eq '1' ? '已开启' : '已关闭'})</b>
        			</li>
       			</div>
		  	</c:if>
	</c:forEach>
	<form name="ptzczhForm" id="ptzczhForm" action="${ctx}/vedsb/ptzh/jpptzczh/saveZh" method="post">
