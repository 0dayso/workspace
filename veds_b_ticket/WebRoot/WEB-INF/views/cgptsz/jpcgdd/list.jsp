...
<script type="text/javascript">
	...
	CGDW ='${vfn:toJSON(vfc:getVeclassLb('100021'))}';
	PTZFFS = '${vfn:toJSON(vfc:getVeclassLb('100023'))}';
</script>
<script id="tpl_list_table" type="text/html">
	...
	<table style="width:2000px" border="0" cellpadding="0" cellspacing="0" class="list_table">
		<tr>
			...
			<th style="width:60px;height:40px">航空公司<br>账号</th>
		...
		{{# for(var i = 0, len = d.list.length; i < len; i++){ }}
			<tr class="tr">
				...
				<td	 class="td_center">{{  $.findJsonId(PTZFFS,d.list[i].paykind).mc }}</td>
				...
				<td  class="td_center">{{  $.nullToEmpty(d.list[i].ex.CGZFKM.kmmc) }}</td>
				<td	 class="td_center">{{  $.nullToEmpty(d.list[i].cgly) }}</td>
				<td	 class="td_center">{{  $.findJsonId(CGDW,d.list[i].cgdw).mc }}</td>
       		...
       		<div class="box">
          		<div class="box_border">
            		<div class="box_center pt10 pb10">
            			<form action="${ctx}/vedsb/cgptsz/jpcgdd/list" id="searchForm" name="searchForm" method="post">
            				<input type="hidden"  name="VIEW" value="cedecf7a73d747b5c2f70af05b294be5" />
            				<input type="hidden"  name="pageNum" value="${param.pageNum }" id="pageNum"/>
            				<input type="hidden"  name="orderBy" value="cjdatetime desc" id="orderBy"/>
              				<table class="table01" border="0" cellpadding="0" cellspacing="0"> 
              					<tr>
              						...
              						<td class="cgdd_left">
              							<input type="text" name="search_GTEdats_cjdatetime" value="${empty param.search_GTE_cjdatetime ? vfn:dateShort() : param.search_GTE_cjdatetime}" style="width:85px"  class="input-text Wdate" size="10" id="starttime" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'endtime\')}'})"/>
              						</td>
              						<td class="cgdd_right">
              							创建日止
              						</td>
              						<td class="cgdd_left">
              							<input type="text" name="search_LTEdatd_cjdatetime" value="${empty param.search_LTE_cjdatetime ? vfn:dateShort() : param.search_LTE_cjdatetime}" style="width:85px"   class="input-text Wdate" size="10" id="endtime" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'starttime\')}'})"/>
              						</td>
              						<td class="cgdd_right">
              							订单编号
              						</td>
              						...
              						<td class="cgdd_left">
              							<input type="text" name="search_EQ_pnrno" value="" onblur="value=$.trim(this.value).toUpperCase();" style="width:85px"  class="input-text" />
              						</td>
              						<td class="cgdd_right">
              							大编码
              						</td>
              						<td class="cgdd_left">
              							<input type="text" name="search_EQ_hkgs_pnrno" value="" onblur="value=$.trim(this.value).toUpperCase();" style="width:85px"  class="input-text" />
              						</td>	
              					<tr>
              						<td class="cgdd_right">
              							采购来源
              						...
              						<td class="cgdd_left">
              							<input type="text" name="search_EQ_hkgs_ddbh" value="" style="width:85px"    class="input-text" />
              						</td>
              						<td class="cgdd_right">
              							交易状态
              						</td>
              						...
              						<td class="cgdd_left">
              							<input type="text" name="search_EQ_trade_no" value="" style="width:85px"   class="input-text" size="10" />
              						</td>
								</tr>
								<tr>
              						<td colspan="9"></td>
