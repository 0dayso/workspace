...
public class DefaultHref implements Href {
	...
	public String toString() {
		...
		if (paramlist != null && paramlist.size() > 0 && "vefun".equals((String) parameters.get("vehref"))
				&& this.url.startsWith("javascript")) {
			String u = this.url;
			if (u.endsWith(")")) {
			...
			if (paramlist.size() > 1) {
				for (int i = 1; i < paramlist.size(); i++) {
					u = u + "'" + parameters.get(paramlist.get(i)) + "'";
	...
	public boolean equals(Object object) {
		...
		return new EqualsBuilder().append(this.parameters, rhs.parameters).append(this.url, rhs.url).append(
				this.anchor, rhs.anchor).isEquals();
	}
