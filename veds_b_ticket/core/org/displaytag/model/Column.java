...
public class Column {
	...
	public Object getValue(boolean decorated) throws ObjectLookupException, DecoratorException {
		...
		if (decorated) {
			for (int j = 0; j < decorators.length; j++) {
				object = decorators[j].decorate(object, row.getParentTable().getPageContext(), row.getParentTable()
						.getMedia());
			}
		}
	...
	public String createChoppedAndLinkedValue() throws ObjectLookupException, DecoratorException {
		...
		if (this.header.getHref() != null) {
			...
			anchor.setTitle(this.cell.getHrefTitle());
			anchor.setOnclick(this.cell.getOnclick());
	...
	private Href getColumnHref(String columnContent) throws ObjectLookupException {
		...
		if (this.header.getParamName() != null) {
			if (this.header.getParamName().startsWith("vehref") || this.header.getParamName().startsWith("vefun")) {
				try {
					String[] vf = this.header.getParamName().split(",");
					colHref.addParameter("vehref", vf[0]);
					if (vf.length > 1) {
						for (int i = 1; i < vf.length; i++) {
							Object paramValue1 = LookupUtil.getBeanProperty(this.row.getObject(), vf[i]);
							String v = "";
							if (paramValue1 != null) {
								v = paramValue1.toString();
							}
							if (this.header.getParamName().startsWith("vehref")) {
								colHref.addParameter(vf[i].toLowerCase(), URLEncoder.encode(v, StringUtils
										.defaultString(this.row.getParentTable().getEncoding(), "UTF8")));
							} else {
								if (NumberUtils.isNumber(vf[i].toLowerCase()) && StringUtils.isBlank(v)) {
									colHref.addParameter(vf[i].toLowerCase(), vf[i].toLowerCase());
								} else {
									colHref.addParameter(vf[i].toLowerCase(), v);
								}
							}
						}
					}
