...
public class TotalTableDecorator extends TableDecorator {
	 ...
	 */
	private String subtotalLabel = "{0} 小计";

	/**
	 * Label used for totals. Default: "Total".
	 */
	private String totalLabel = " 合计 ";
	...
	public String startRow() {
		...
		List<Row> l = tableModel.getRowListFull();
		int i=0;
		int rowNum = this.getViewIndex();
		
		for (Iterator it = tableModel.getHeaderCellList().iterator(); it.hasNext();) {
			HeaderCell cell = (HeaderCell) it.next();
			if (cell.isTotaled()) {
				Number amount=0;
				boolean isContentTotaled = false;
				
				String totalPropertyName = cell.getBeanPropertyName();
				Cell contentcell =(Cell) l.get(rowNum).getCellList().get(i);
				isContentTotaled = contentcell.isUserValue();
				
				if(isContentTotaled){
					amount = NumberUtils.toDouble(contentcell.getStaticValue()+"",0) ; 
				}else{
					amount = toNumber(evaluate(totalPropertyName));
				}
				Number previousSubTotal = toNumber(subTotals.get(totalPropertyName));
				Number previousGrandTotals = toNumber(grandTotals.get(totalPropertyName));

				subTotals.put(totalPropertyName, new Double((previousSubTotal != null ? previousSubTotal.doubleValue()
						: 0)
	...
	protected String createTotalRow(boolean grandTotal) {
		StringBuffer buffer = new StringBuffer(1000);
		buffer.append("\n<tr class=\"total\"  style=\"text-align:right;\">"); //$NON-NLS-1$
		...
		for (Iterator it = headerCells.iterator(); it.hasNext();) {
			...
			if (cell.isTotaled()) {
				...
				buffer.append(grandTotal ? totalLabel : MessageFormat.format(subtotalLabel,
	...
	private Number toNumber(Object o) {
		...
		try {
			s = o.toString();
			rnt = org.apache.commons.lang.math.NumberUtils.toDouble(s, 0);
			return rnt;
		} catch (Exception e) {
			return 0;
		}
