...
System.out.println( VeDate.formatToStr(new Date(), "yyyy-M-d"));
	...
	public static List<Date[]> dateExclude(Date fromDate, Date toDate, String str) {
		...
		int pre = VeDate.getTwoDay(toDate, fromDate);
		for (int i = 0; i <= pre; i++) {
			dset.add(VeDate.getPreDay(fromDate, i));
		}
		...
		for (String s : strsz) {
			...
			if (ssz.length > 0) {
				d1 = VeDate.strToDate(ssz[0]);
			}
			if (ssz.length > 1) {
				d2 = VeDate.strToDate(ssz[1]);
			}
			if (d1 == null && d2 == null) {
				continue;
			}
			...
			int p = VeDate.getTwoDay(d2, d1);

			for (int i = 0; i <= p; i++) {
				dset.remove(VeDate.getPreDay(d1, i));
			}
		}
		...
		while (iter.hasNext()) {
			...
			if (da[0] == null) {
				...
				if (dd.equals(VeDate.getPreDay(da[1], 1))) {
					da[1] = dd;
				} else {
					list.add(da);
					da = new Date[2];
	...
	public static String Ftmwk(String tm) {
		...
		if (c > 0) {
			Date d = VeDate.formatToDate(tm, "HH:mm");
			tm = VeDate.formatToStr(d, "HH:mm");
		}
		if (tm == null) {
			return "00:00";
		}
	...
	/**
	 * 判断是否是周末
	 * @param sdate 将长时间格式时间 yyyy-MM-dd HH:mm:ss
	 * @return 周六、周日 true 
	 */
