...
public class ExcelView extends BaseExportView {
	...
	protected String escapeColumnValue(Object value) {
		if (value != null) {
			String vs = StringUtils.trimToEmpty(value.toString());

			vs = VeStr.unHtml(vs);
			vs = DisplayUtils.unTitle(vs);
			if (vs.startsWith("0") || (NumberUtils.isDigits(vs) && vs.length() > 10)) {
				vs = "\t" + vs;
			}
			vs = "\"" + StringUtils.replace(vs, "\"", "\"\"") //$NON-NLS-1$ //$NON-NLS-2$ 
					+ "\"";
