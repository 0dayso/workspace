...
public class TableTag extends HtmlTableTag {
	...
	private String allTitle;// 所有的title用逗号分隔
	private String allExpfield;// 所有导出的列名
	
	private Integer xh;

	private String delcols = "";// 所有不显示的列序号用逗号隔开从0开始
	...
	private void initParameters() throws JspTagException, FactoryInstantiationException {
		...
		if (!this.tableModel.isLocalSort()) {
			...
			Integer sortColumnParameter = getFromRequestOrSession(request, requestHelper,
		...
		if (this.paginatedList == null) {
			SortOrderEnum paramOrder = SortOrderEnum.fromCode(getFromRequestOrSession(request, requestHelper,
		...
		this.currentMediaType = (MediaTypeEnum) ObjectUtils.defaultIfNull(MediaTypeEnum.fromCode(exportTypeParameter),
				MediaTypeEnum.HTML);
		...
		if (this.partialList) {
			...
			if (size == null) {
				...
				throw new JspTagException(Messages.getString("InvalidTypeException.msg", new Object[] { "size",
						"Integer" }));
			}
	...
	protected void initHref(RequestHelper requestHelper) {
		...
		if(StringUtils.isNotBlank(requestUri) && !requestUri.contains("dtableid=")){
			if(requestUri.contains("?")){
				requestUri+="&dtableid="+uid;
			}else {
				requestUri+="?dtableid="+uid;
			}
		}
		...
		if (this.requestUri != null) {
			...
			if (!this.dontAppendContext) {
				...
				if (!StringUtils.isEmpty(contextPath) && requestUri != null && requestUri.startsWith("/")
	...
	public int doEndTag() throws JspException {
        ...
        sortFields.add(new BeanComparator("xh")); // id逆序 (主)
        ComparatorChain multiSort = new ComparatorChain(sortFields);
        Collections.sort(headers, multiSort);
        
        
        //List ordering
        List rows = tableModel.getRowListFull();
        for (Iterator iterator = rows.iterator(); iterator.hasNext();) {
            Row onerow = (Row) iterator.next();
            List cesList= onerow.getCellList();
            int j=100000;
            for (Iterator iteratorRow = cesList.iterator(); iteratorRow.hasNext();) {
                Cell Cell = (Cell) iteratorRow.next();
                if(Cell.getXh()==null){
                    Cell.setXh(j);
                }
                j++;
            ...
            Collections.sort(cesList, multiSort);
        }
		

		//request.setAttribute(this.uid + "expfield", DisplayUtils.getExp(this.tableModel.getHeaderCellList()));

		request.setAttribute(this.uid + "alltitle", this.getAllTitle());
		request.setAttribute(this.uid + "allexpfield", this.getAllExpfield());
		
		request.setAttribute(this.uid + "expfield", DisplayUtils.getExp2(this.getAllTitle(),this.getAllExpfield()));
		...
		TableDecorator tableDecorator = this.properties.getDecoratorFactoryInstance().loadTableDecorator(
				this.pageContext, getConfiguredDecoratorName());
		...
		String mergeid = StringUtils.trimToEmpty(uid) + "mergeid";
		String mergetable = (String) request.getAttribute(mergeid);

		if (StringUtils.isNotBlank(mergetable) && StringUtils.isNotBlank(uid)) {
			JspWriter out = this.pageContext.getOut();
			try {
				out.write("<script type=\"text/javascript\">try{$.mergeTable('" + uid + "',[" + mergetable
						+ "]);}catch(e){}</script>");
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		if (StringUtils.isNotBlank(getDelcols())) {
			JspWriter out = this.pageContext.getOut();
			try {
				out.write("<script type=\"text/javascript\">try{mooop('" + uid + "','" + getDelcols().substring(1)
						+ "');}catch(e){}</script>");
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	protected int doExport() throws JspException {
		...
		ExportView exportView = ExportViewFactory.getInstance().getView(this.currentMediaType, this.tableModel,
	...
	protected void writeHTMLData() throws JspException {
		...
		new HtmlTableWriter(this.tableModel, this.properties, this.baseHref, this.export, out, getCaptionTag(),
				this.paginatedList, this.listHelper, this.pagesize, getAttributeMap(), this.uid).writeTable(
				this.tableModel, this.getUid());
	...
	public String getDelcols() {
		return delcols;
	}

	public void setDelcols(String delcols) {
		this.delcols = delcols;
	}

	public String getAllExpfield() {
		return allExpfield;
	}

	public void setAllExpfield(String allExpfield) {
		this.allExpfield = allExpfield;
	}
