...
class ResultSetWrapper {
  ...
  public ResultSetWrapper(ResultSet rs, Configuration configuration) throws SQLException {
    ...
    for (int i = 1; i <= columnCount; i++) {
      columnNames.add(configuration.isUseColumnLabel() ? metaData.getColumnLabel(i).toUpperCase(Locale.ENGLISH) : metaData.getColumnName(i).toUpperCase(Locale.ENGLISH));
  ...
  private void loadMappedAndUnmappedColumnNames(ResultMap resultMap, String columnPrefix) throws SQLException {
    ...
    System.out.println("列的映射，获取列"+columnNames);
    for (String columnName : columnNames) {
      final String upperColumnName = columnName.toUpperCase(Locale.ENGLISH);
      //System.out.println("列的映射"+mappedColumns);
